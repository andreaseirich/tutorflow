...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        blocked_time_ids = list(
            BlockedTime.objects.order_by("-created_at").values_list("id", flat=True)[:2]
        )
        ...
        if dry_run:
            ...
            deleted_count, deleted_objects = BlockedTime.objects.filter(
                id__in=blocked_time_ids
            ).delete()
